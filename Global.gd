...
func save_score():
	...
	save_score.open("user://playerScore.save", File.WRITE)
	...
	for node in save_nodes:
		...
		# Store the save dictionary as a new line in the save file.
		save_score.store_line(to_json(node_data))
	save_score.close()

func save_name():
	var save_name = File.new()
	save_name.open("user://playerName.save", File.WRITE)
	var save_nodes = get_tree().get_nodes_in_group("PlayerName")
	for node in save_nodes:
		# Check the node is an instanced scene so it can be instanced again during load.
		if node.filename.empty():
			print("persistent node '%s' is not an instanced scene, skipped" % node.name)
		...
		# Store the save dictionary as a new line in the save file.
		save_name.store_line(to_json(node_data))
	save_name.close()
